...
class UserExport implements WithMapping,Responsable,WithHeadings,FromCollection,WithEvents,ShouldAutoSize
{
    ...
    {
        $governorate = $this->req->get('governorate_id', false);
        $name = $this->req->get('name', false);

        $users = User::query()->with(['userInfo', 'userInfo.birthGovernorate', 'userInfo.governorate'])
            ->when($name, function (Builder $query) use ($name) {
                $query->where('name', 'like', '%' . $name . '%');
            })
            ->when($governorate, function (Builder $query) use ($governorate){
                $query->whereRelation('userInfo', 'governorate_id', $governorate);
            })
            ->latest()
            ->get();
